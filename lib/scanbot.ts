...
export default class ScanbotSDKService {
  ...
  async createBarcodeScanner(
    containerId: string,
    onBarcodeFound: (code: BarcodeItem) => void,
  ) {
    /*
     * Ensure the SDK is initialized. If it's initialized, this function does nothing,
  ...
  async applyCrop(id: string) {
    ...
    if (scanningResult) {
      ...
      (scanningResult.detectionResult as any).pointsNormalized =
        result?.polygon ?? [];
    }

    const jpegResult = await ScanbotSDKService.instance.sdk!.imageToJpeg(
      result!.image,
    );
    document.image = await ScanbotSDKService.instance.sdk!.toDataUrl(
      jpegResult as unknown as ArrayBuffer,
    );

...
export default function SearchInput({ placeholder, paramKey }: InputProps) {
  ...
  const updateQuery = useDebouncedCallback((value: string) => {
    const params = new URLSearchParams(searchParams);
    if (value) {
      params.set(`${paramKey}query`, value);
    } else {
      params.delete(`${paramKey}query`);
    }
    replace(`${pathname}?${params.toString()}`);
  ...
  // Whenever user types, update state + debounce update
  ...
  return (
    <Input
      ...
      className="border-primary text-foreground w-fit border-2 py-4 text-right md:w-60"
      defaultValue={searchParams.get(`${paramKey}query`)?.toString()}
    />
  );
}

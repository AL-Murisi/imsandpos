  ...
  const summaryCards = [
    {
      ...
      subtitle: `${salesCount.toLocaleString("en-US")} عملية بيع`,
      icon: ShoppingCart,
    ...
    },
    {
      ...
      subtitle: `${salesDue.toLocaleString("en-US")} متبقي على المبيعات`,
      icon: ReceiptText,
      tone: "from-amber-50 to-white border-amber-100",
      iconWrap: "bg-amber-100 text-amber-700",
    },
    {
      title: "العمليات غير المكتملة",
      value: (statusCounts.partial + statusCounts.unpaid).toLocaleString("en-US"),
      subtitle: `${statusCounts.paid.toLocaleString("en-US")} عملية مدفوعة بالكامل`,
      icon: CreditCard,
          ...
          <div className="grid gap-6 p-4 md:p-7 lg:grid-cols-[1.6fr_1fr]">
            <div className="min-w-0 space-y-4">
              <Badge className="w-fit max-w-full bg-white/12 text-white hover:bg-white/12">
                {scope === "company" ? "لوحة مبيعات عامة" : "لوحة الكاشير"}
              </Badge>
              <div className="space-y-2">
                <h2 className="text-xl font-semibold md:text-3xl">
                  متابعة المبيعات والتحصيل في شاشة واحدة
                </h2>
                <p className="max-w-2xl text-sm leading-6 text-slate-200 md:text-base">
                  راقب أداء البيع اليومي، والمبالغ المقبوضة، والعمليات المتبقية،
                  وآخر الفواتير من نفس صفحة العمل.
                </p>
              </div>
              <div className="flex flex-wrap gap-2 text-sm text-slate-100 md:gap-3">
                ...
                <div className="max-w-full rounded-full border border-white/15 bg-white/8 px-3 py-1.5 text-xs sm:text-sm">
                  {statusCounts.partial.toLocaleString("en-US")} فواتير جزئية
                </div>
                <div className="max-w-full rounded-full border border-white/15 bg-white/8 px-3 py-1.5 text-xs sm:text-sm">
                  {dueSales.length.toLocaleString("en-US")} عملاء بحاجة متابعة
                </div>
              </div>
            </div>
            ...
            <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <div className="rounded-2xl border border-white/12 bg-white/8 p-4 backdrop-blur-sm">
                ...
                <p className="mt-2 text-sm text-slate-300">
                  نسبة النقد أو التحصيل المسجل مقارنة بالمبيعات اليومية.
                </p>
              </div>
              <div className="rounded-2xl border border-white/12 bg-white/8 p-4 backdrop-blur-sm">
                <div className="mb-3 flex items-center justify-between">
                  <span className="text-sm text-slate-200">فواتير تحتاج متابعة</span>
                  <Users className="h-4 w-4 text-slate-200" />
                </div>
                <div className="text-3xl font-semibold">
                  {(statusCounts.partial + statusCounts.unpaid).toLocaleString("en-US")}
                </div>
                <p className="mt-2 text-sm text-slate-300">
                  عمليات لم تكتمل بالكامل وتحتاج متابعة مع العميل.
                </p>
              </div>
            </div>
          </div>
          ...
          <Card className="min-w-0 border-0 bg-white/90 shadow-sm">
            <CardHeader className="pb-2">
              ...
              <p className="text-sm text-slate-500">
                آخر العمليات المسجلة مع الحالة والمبلغ.
              </p>
            </CardHeader>

...
//       // إذا كانت العملة المختارة هي الريال اليمني (الأساسية)
//       if (currency.id === company.base_currency) {
//         setExchangeRate(1);
//         setReceivedAmount(totals.totalAfter); // اطلب المبلغ بالريال كما هو
//         return;
//       }
...
//           fromCurrency: company.base_currency, // YER
//           toCurrency: currency.id, // USD
//         });
...
//           // المنطق الصحيح: تحويل إجمالي السلة (YER) إلى العملة المختارة (USD)
//           // إذا كان سعر الصرف المخزن هو 0.00066 (سعر الدولار مقابل الريال)
//           // أو إذا كان السعر مخزن كـ 1500، سنقسم عليه

//           const autoAmount = totals.totalAfter * rateValue;
//           setReceivedAmount(Number(autoAmount.toFixed(2)));
//         }
//       } catch (error) {
...
//   const currentTotalInBase = isForeign
//     ? receivedAmount * exchangeRate
//     : receivedAmount;

//   const canPay =
//     (receivedAmount > 0 && currentTotalInBase >= totals.totalAfter - 0.01) || // سماحية بسيطة للكسور العشرية
//     (selectedUser?.id && currentTotalInBase < totals.totalAfter); // السماح بالدفع الجزئي إذا وُجد عميل
...
//                     تعادل: {(receivedAmount / exchangeRate).toLocaleString()}{" "}
//                     {company?.base_currency}
//                   </div>
//                 )}
//               </div>
